...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1920)#分辨率设置为1080X1920
...
if __name__ == "__main__":
    ...
    while Status != Status_t.Finished:
        if Status == Status_t.start:
            ...
            key = cv2.waitKey(1) # 在1ms内根据键盘输入返回一个值，没有按键返回-1，有按键返回ASSCII码
            if key == ord("q"):
                break
        elif Status == Status_t.findQRCode:
            qrCodeDetector = cv2.QRCodeDetector() # 设置二维码检测器
            ret, img = cap.read()
            cv2.imshow("original", img)
            cv2.waitKey(1)
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            for barcode in decode(gray):
                print(barcode.data.decode('utf-8')) # 二维码数据信息
                ...
                pass
            pass
        elif Status == Status_t.findCGofCyc: # 圆环
            ret, image = cap.read()
            ...
            if CycCGs.RedX == 0:
                ...
                retVal, thresh = cv2.threshold(new_r, 40, 255, cv2.THRESH_BINARY) # 二值化
            ...
            if contours: # 如果图像中有轮廓
                # find the biggest countour (c) by the area
                c = max(contours, key=cv2.contourArea) # 轮廓区域面积最大
                # print(cv2.contourArea(c))
                if cv2.contourArea(c) > 50:
                    M = cv2.moments(c)
                    cx = int(M['m10'] / M['m00']) # 圆环x方向重心
                    if CycCGs.RedX == 0:
                        CycCGs.RedX = cx
                    elif CycCGs.GreenX == 0:
                        CycCGs.GreenX = cx
            ...
            pass
        elif Status == Status_t.findCGofRec: # 物品
            ret, image = cap.read()
            # image = cv2.imread("a.jpg")
