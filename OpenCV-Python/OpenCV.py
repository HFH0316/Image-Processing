...
if __name__ == '__main__': 
    ...
    while(1):
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'): # 按下q时截取图片
    ...
    #! 梯度灰度
    img = myGradient(img)
    cv2.namedWindow('img_gradient', cv2.WINDOW_NORMAL)
    cv2.imshow('img_gradient', img)
    '''
    '''
    #! 伽马变换
    fi = img / 255.0 # 8位图归一化处理（将灰度图像素值调整到[0,1]之间）
    gamma = 0.5 # 0 < gamma < 1降低对比度 gamma > 1增加对比度
    ...
    # 中值滤波
    img_median = cv2.medianBlur(img, 5) # ksize滤波模板的尺寸大小，必须是大于1的奇数
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, 5, minLineLength=5, maxLineGap=5)
    for line in lines:
        x1 = line[0][0]
        y1 = line[0][1]
        x2 = line[0][2]
    ...
    '''
    #! 霍夫圆检测
    circles = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 1, 200, param1=100, param2=30, minRadius=20, maxRadius=300)
